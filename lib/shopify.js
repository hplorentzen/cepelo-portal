...
// Storefront: resolve a file-reference GID (Video/ExternalVideo) to a URL
...
function normaliseRecommendation(node) {
  ...
  return {
    ...
    description: {},
    net_price:   Math.round(grossPrice),    // variant.price = dealer cost; overridden nowhere for accessories
    gross_price: Math.round(grossPrice),    // enriched with vejl_udsalgspris metafield below
...
export async function fetchProductBySku(sku) {
  ...
  if (numericId) {
    try {
      ...
      // 2. video_url: "produktvideo" file_reference metafield
      const videoMf = metafields.find(
        m => m.namespace === 'custom' && m.key === 'produktvideo'
      )
      if (videoMf?.value) {
        const val = String(videoMf.value)
        if (val.startsWith('http')) {
          // Direct URL stored in metafield
          product.video_url = val
        } else if (val.startsWith('gid://')) {
          // File reference GID — resolve to URL via Storefront node query
          try {
            const fileData = await storefrontQuery(FILE_NODE_QUERY, { id: val })
            const n = fileData?.node
            if (n?.sources?.length) {
              const mp4 = n.sources.find(s => s.mimeType === 'video/mp4')
              product.video_url = (mp4 || n.sources[0]).url
            } else if (n?.embeddedUrl) {
              product.video_url = n.embeddedUrl
            }
          } catch (e) {
            console.warn(`[shopify/metafields] video GID ${val} resolution failed:`, e.message)
          }
        }
      }
    } catch (e) {
...
export async function fetchAccessoriesFromMetafields(productGid) {
  ...
  try {
    ...
    // Done in parallel to minimise latency; failures fall back to variant.price silently.
      ...
      accessories.map(async acc => {
        ...
        try {
          ...
          if (grossMf?.value) {
            const v = parseFloat(String(grossMf.value).replace(',', '.'))
            if (!isNaN(v) && v > 0) {
              console.log(`[shopify/metafields] acc gross_price for ${acc.sku}: ${Math.round(v)} (from metafield)`)
              return { ...acc, gross_price: Math.round(v) }
            }
          }
        } catch (e) {
          console.warn(`[shopify/metafields] acc enrichment failed for ${acc.sku}:`, e.message)
        }
        return acc
      })
    )
