...
// Media is fetched separately so a scope/auth failure cannot break the
// main product query (images, prices, SKU match).
...
export async function fetchProductBySku(sku) {
  ...
  // Media is a separate, optional query — a failure here must never affect
  // image_url, gross_price, or any other core field.
  try {
    const mediaData  = await storefrontQuery(PRODUCT_MEDIA_QUERY, { productId: node.id })
    const mediaEdges = mediaData?.node?.media?.edges ?? []
    for (const { node: m } of mediaEdges) {
      if (m.mediaContentType === 'VIDEO' && m.sources?.length) {
        const mp4 = m.sources.find(s => s.mimeType === 'video/mp4')
        product.video_url = (mp4 || m.sources[0]).url
        break
      }
      if (m.mediaContentType === 'EXTERNAL_VIDEO' && m.embeddedUrl) {
        product.video_url = m.embeddedUrl
        break
      }
    }
  } catch (e) {
    console.warn(`[shopify] Media fetch failed for ${sku} (video_url will be null):`, e.message)
  }
