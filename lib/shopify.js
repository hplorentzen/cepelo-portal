...
function normaliseProduct(node, targetSku) {
  ...
  // Extract first video URL (native Video preferred, ExternalVideo/embed as fallback)
  let video_url = null
  for (const { node: m } of (node.media?.edges ?? [])) {
    if (m.mediaContentType === 'VIDEO' && m.sources?.length) {
      const mp4 = m.sources.find(s => s.mimeType === 'video/mp4')
      video_url = (mp4 || m.sources[0]).url
      break
    }
    if (m.mediaContentType === 'EXTERNAL_VIDEO' && m.embeddedUrl) {
      video_url = m.embeddedUrl
      break
    }
  }
