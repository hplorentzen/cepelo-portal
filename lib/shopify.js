...
export async function fetchProductByTitle(title) {
  ...
  let data
  try {
    // Use title: prefix so Shopify restricts the search to product titles
    // rather than doing a general full-text search (which can surface
    // unrelated products that share one word with the bundle name).
    data = await storefrontQuery(PRODUCT_BY_SKU_QUERY, { query: `title:${title}` })
  } catch (e) {
  ...
  // Guard against false positives: require ≥50% of the significant words
  // from the searched title to appear in the returned product title.
  // "Significant" = length ≥ 3 after stripping punctuation.
  // This catches cases like searching "Maxisys Ultra S2 inkl. scope" and
  // Shopify returning "Autel Maxisys 919S2" (only 1/4 words match → skipped).
  const sigWords = title.toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter(w => w.length >= 3)
  const resultTitle = node.title.toLowerCase()
  const matchCount = sigWords.filter(w => resultTitle.includes(w)).length
  const matchRatio = sigWords.length > 0 ? matchCount / sigWords.length : 0
  if (matchRatio < 0.5) {
    console.warn(
      `[shopify/fetchProductByTitle] mismatch: searched "${title}", got "${node.title}" ` +
      `(${Math.round(matchRatio * 100)}% word match) — skipping`
    )
    return null
  }
